...
func (a *PublicAPIService) GetCmsV3SiteSearchSearchSearchExecute(r ApiGetCmsV3SiteSearchSearchSearchRequest) (*PublicSearchResults, *http.Response, error) {
	...
	if r.matchPrefix != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "matchPrefix", r.matchPrefix, "form", "")
	}
	if r.autocomplete != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "autocomplete", r.autocomplete, "form", "")
	}
	if r.popularityBoost != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "popularityBoost", r.popularityBoost, "form", "")
	}
	if r.boostLimit != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "boostLimit", r.boostLimit, "form", "")
	}
	if r.boostRecent != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "boostRecent", r.boostRecent, "form", "")
	}
	if r.tableId != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "tableId", r.tableId, "form", "")
	}
	if r.hubdbQuery != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "hubdbQuery", r.hubdbQuery, "form", "")
	}
	if r.domain != nil {
		t := *r.domain
	...
	if r.pathPrefix != nil {
		...
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				parameterAddToHeaderOrQuery(localVarQueryParams, "pathPrefix", s.Index(i).Interface(), "form", "multi")
			}
		} else {
			parameterAddToHeaderOrQuery(localVarQueryParams, "pathPrefix", t, "form", "multi")
		}
	}
	if r.property != nil {
		t := *r.property

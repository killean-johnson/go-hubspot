# Go API client for commerce_subscriptions
...
```go
import commerce_subscriptions "github.com/GIT_USER_ID/GIT_REPO_ID"
```
...
For using other server than the one defined on index 0 set context value `commerce_subscriptions.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), commerce_subscriptions.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `commerce_subscriptions.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), commerce_subscriptions.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `commerce_subscriptions.ContextOperationServerIndices` and `commerce_subscriptions.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), commerce_subscriptions.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), commerce_subscriptions.ContextOperationServerVariables, map[string]map[string]string{
...
------------ | ------------- | ------------- | -------------
*BasicAPI* | [**GetCrmV3ObjectsSubscriptions**](docs/BasicAPI.md#getcrmv3objectssubscriptions) | **Get** /crm/v3/objects/subscriptions | Retrieve subscriptions
*BasicAPI* | [**GetCrmV3ObjectsSubscriptionsSubscriptionId**](docs/BasicAPI.md#getcrmv3objectssubscriptionssubscriptionid) | **Get** /crm/v3/objects/subscriptions/{subscriptionId} | Retrieve a subscription
*SearchAPI* | [**PostCrmV3ObjectsSubscriptionsSearch**](docs/SearchAPI.md#postcrmv3objectssubscriptionssearch) | **Post** /crm/v3/objects/subscriptions/search | Search for subscriptions
...
```go
auth := context.WithValue(context.Background(), commerce_subscriptions.ContextAccessToken, "ACCESSTOKENSTRING")
...
auth := context.WithValue(oauth2.NoContext, commerce_subscriptions.ContextOAuth2, tokenSource)
r, err := client.Service.Operation(auth, args)
```
...
auth := context.WithValue(
		context.Background(),
		commerce_subscriptions.ContextAPIKeys,
		map[string]commerce_subscriptions.APIKey{

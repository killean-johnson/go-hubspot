# Go API client for commerce_payments
...
```go
import commerce_payments "github.com/GIT_USER_ID/GIT_REPO_ID"
```
...
For using other server than the one defined on index 0 set context value `commerce_payments.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), commerce_payments.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `commerce_payments.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), commerce_payments.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `commerce_payments.ContextOperationServerIndices` and `commerce_payments.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), commerce_payments.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), commerce_payments.ContextOperationServerVariables, map[string]map[string]string{
...
------------ | ------------- | ------------- | -------------
*BasicAPI* | [**GetCrmV3ObjectsCommercePayments**](docs/BasicAPI.md#getcrmv3objectscommercepayments) | **Get** /crm/v3/objects/commerce_payments | Retrieve payments
*BasicAPI* | [**GetCrmV3ObjectsCommercePaymentsCommercePaymentId**](docs/BasicAPI.md#getcrmv3objectscommercepaymentscommercepaymentid) | **Get** /crm/v3/objects/commerce_payments/{commercePaymentId} | Read
*BatchAPI* | [**PostCrmV3ObjectsCommercePaymentsBatchRead**](docs/BatchAPI.md#postcrmv3objectscommercepaymentsbatchread) | **Post** /crm/v3/objects/commerce_payments/batch/read | Retrieve a batch of payments
*SearchAPI* | [**PostCrmV3ObjectsCommercePaymentsSearch**](docs/SearchAPI.md#postcrmv3objectscommercepaymentssearch) | **Post** /crm/v3/objects/commerce_payments/search | Search for payments
...
```go
auth := context.WithValue(context.Background(), commerce_payments.ContextAccessToken, "ACCESSTOKENSTRING")
...
auth := context.WithValue(oauth2.NoContext, commerce_payments.ContextOAuth2, tokenSource)
r, err := client.Service.Operation(auth, args)
```
...
auth := context.WithValue(
		context.Background(),
		commerce_payments.ContextAPIKeys,
		map[string]commerce_payments.APIKey{

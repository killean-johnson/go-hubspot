...
```go
import notes "github.com/GIT_USER_ID/GIT_REPO_ID"
```
...
For using other server than the one defined on index 0 set context value `notes.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), notes.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `notes.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), notes.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `notes.ContextOperationServerIndices` and `notes.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), notes.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), notes.ContextOperationServerVariables, map[string]map[string]string{
...
```go
auth := context.WithValue(context.Background(), notes.ContextAccessToken, "ACCESSTOKENSTRING")
...
auth := context.WithValue(oauth2.NoContext, notes.ContextOAuth2, tokenSource)
r, err := client.Service.Operation(auth, args)
```
...
auth := context.WithValue(
		context.Background(),
		notes.ContextAPIKeys,
		map[string]notes.APIKey{
